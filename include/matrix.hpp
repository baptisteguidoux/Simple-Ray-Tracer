...
namespace math {
  ...
  struct Matrix {
     ...
     * Get the value located at the given position
     * \param row row index
   ...
   * Compare the equality of two Matrix
   * \param a a Matrix
  ...
  /* \fn bool operator!=(const Matrix& a, const Matrix& b)
   * Compare the unequality of two Matrix
   * \param a a Matrix
   * \param b another Matrix
   ...
   * Transposes a Matrix, ie rows become columns
   * \param matrix a Matrix
   * \return A new Matrix
   */     
   ...
   * Removes the row at pos row_pos, the column at col_pos from the input matrix
   * \param matrix a Matrix
   ...
   * Computes the given Matrix's determinant, useful to inverse a Matrix
   * \param matrix a Matrix 
   ...
   * Cofactors are minors who may have their sign changed (if row + col is odd --> negate the minor)
   * Useful to compute Matrix's inverse
   * \param row_pos element row position
   * \param col_pos element column position
   ...
   * Inverse the Matrix (better use is_invertible(const Matrix& matrix) before!)
   * \param matrix a Matrix
   ...
   * Builds a rotation Matrix on the x axis
   ...
   * Builds a rotation Matrix on the y axis
   * \param radians the rotation angle
   ...
   * Builds a rotation Matrix on the z axis
   * \param radians the rotation angle
   * \return a rotation Matrix
   */      
  ...
  * Moves an axis in proportion of another
  *  \param x_y
  *  \param x_z
  *  \param y_x
  *  \param y_z
